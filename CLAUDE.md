...
- Тексты описаний — `ReplicatedStorage/WikiTexts.luau`, **генерируется** `python3 tools/wiki_texts.py`
  из `ВИКИПЕДИЯ/` (effects, enemies). Руками не править: поменялась вики — перезапустить скрипт.
